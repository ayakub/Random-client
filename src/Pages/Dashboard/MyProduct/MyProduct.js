...
const MyProduct = () => {
    ...
    const { data: myProducts = [], isLoading, refetch } = useQuery({
        queryKey: ['repoData'],
        queryFn: () =>
            fetch(`https://reused-product-sell-server-ayakub.vercel.app/addproduct?email=${user.email}`).then(res =>
                res.json()
            )
    })
    const handdleDeleteProduct = id => {
        ...
        if (permision) {
            fetch(`https://reused-product-sell-server-ayakub.vercel.app/myprosuct/${id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(data => {
